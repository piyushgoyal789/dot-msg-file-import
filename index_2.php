...
// Function to display MSG file contents
function displayMsgContents($msgData) {
    if ($msgData === null) {
        return;
    }

    echo "<div style='font-family: Arial, sans-serif; margin: 20px;'>\n";
    echo "<h2>MSG File Contents</h2>\n";
    
    // Display headers
    echo "<div style='border: 1px solid #ccc; padding: 15px; margin-bottom: 20px;'>\n";
    echo "<h3>Email Headers</h3>\n";
    
    if (isset($msgData->headers) && !empty($msgData->headers)) {
        foreach ($msgData->headers as $key => $value) {
...
<head>
    ...
    <title>MSG File Reader (FC Version)</title>
    <style>
...
<body>
    <div class="container">
        <h1>MSG File Parser & Database (FC Version)</h1>
        <p style="color: #666; font-style: italic;">FC Version - Enhanced for subject-based passenger extraction</p>
        ...
        <div style="margin: 20px 0;">
            ...
            <a href="index.php" style="background: #6c757d; color: white; padding: 10px 20px; text-decoration: none; border-radius: 4px; display: inline-block; margin-left: 10px;">🔙 Back to Original</a>
        </div>
        
        <div class="upload-form">
            <h3>Upload New Format MSG Files</h3>
...
try {
    ...
    echo "<ul>";
    echo "<li><strong>Total PDF Attachments:</strong> " . ($stats['total_attachments'] ?? 0) . "</li>";
    echo "<li><strong>Unique Passengers (PDF):</strong> " . ($stats['unique_passengers'] ?? 0) . "</li>";
...
?>
        ...
        <div style="margin-top: 30px; padding: 15px; background: #e6f3ff; border-radius: 6px;">
            <h3>Version 2 Features:</h3>
            <ul>
                <li>Enhanced passenger name extraction from email subjects</li>
                <li>Supports new MSG file formats like "Flight Ticket PNR_ 1877411 Passenger Name_ PELEG MOSHE (1)"</li>
                <li>Upload-based processing (no default files)</li>
                <li>Same database functionality as original</li>
                <li>Subject-priority extraction: Subject → Body → PDF content → Filename</li>
            </ul>
            
            <h4>Usage:</h4>
            <ol>
                <li>Upload your MSG file using the form above</li>
                <li>System will automatically extract passenger name from email subject</li>
                <li>View results in database viewer</li>
            </ol>
        </div>
    </div>
</body>
