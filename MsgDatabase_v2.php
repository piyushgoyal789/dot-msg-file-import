...
class MsgDatabase_v2 {
    ...
    /**
     * Extract passenger name from attachment filename or content (V2 - Subject Priority)
     */
    public function extractPassengerName($filename, $content = '', $emailBodyText = '', $emailSubject = '') {
        // First, try to extract from email subject (highest priority for new format)
        if (!empty($emailSubject)) {
            $patterns = [
                // Pattern for "Flight Ticket PNR_ 1877411 Passenger Name_ PELEG MOSHE (1)" format
                '/Passenger Name[_:\s]*([A-Z\s]+(?:\([0-9]+\))?)/i',
                // Pattern for "Passenger: Name" format in subject
                '/Passenger[_:\s]+([A-Za-z\s]+(?:Mrs?|Ms|Miss|Dr)\.?)/i',
                // General name pattern after "Name" keyword
                '/Name[_:\s]+([A-Za-z\s]+)/i'
            ];
            
            foreach ($patterns as $pattern) {
                if (preg_match($pattern, $emailSubject, $matches)) {
                    $name = trim($matches[1]);
                    // Clean up the name - remove numbers and parentheses
                    $name = preg_replace('/\([0-9]+\)/', '', $name); // Remove (1), (2), etc.
        ...
        // Second, try to extract from email body text
        ...
        // Third, try to extract from decoded PDF content if it's a PDF
        if (!empty($content)) {
        ...
        try {
            ...
            if (!empty($attachments)) {
                foreach ($attachments as $attachment) {
                    ...
                    $passengerName = $this->extractPassengerName($filename, $data, $emailData['body_text'], $emailData['subject']);
    ...
    public function getPassengerAttachments($passengerName = null) {
        $sql = "SELECT * FROM passenger_attachments WHERE attachment_type = 'application/pdf'";
        $params = [];
    ...
    /**
     * Get attachment statistics (PDF only)
     */
    public function getAttachmentStats() {
        $sql = "SELECT 
                ...
                WHERE attachment_type = 'application/pdf'";
