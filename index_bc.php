...
<body>
    <div class="container">
        <h1>MSG File Parser & Database</h1>
        ...
        <div class="upload-form">
            <h3>Upload MSG File</h3>
            <form method="POST" enctype="multipart/form-data">
                <input type="file" name="msg_file" accept=".msg" required>
                <br>
                <button type="submit" name="upload">Read MSG File</button>
            </form>
        </div>

<?php
// Handle file upload and processing
if (isset($_POST['upload']) && isset($_FILES['msg_file'])) {
    $uploadedFile = $_FILES['msg_file'];
    
    // Check for upload errors
    if ($uploadedFile['error'] !== UPLOAD_ERR_OK) {
        echo "<div class='error'>Upload failed with error code: " . $uploadedFile['error'] . "</div>";
    } else {
        ...
        if (move_uploaded_file($uploadedFile['tmp_name'], $targetPath)) {
            echo "<div class='success'>File uploaded successfully: " . htmlspecialchars($fileName) . "</div>";
            ...
            try {
                ...
                if ($msgData) {
                    ...
                    echo "<strong>Success!</strong> MSG file processed and saved to database. Email ID: " . $emailId;
                    echo "</div>";
                    
                    displayMsgContents($msgData);
                } else {
                    echo "<div class='error'>Failed to parse MSG file.</div>";
                }
            } catch (Exception $e) {
                echo "<div class='error'>Error processing MSG file: " . htmlspecialchars($e->getMessage()) . "</div>";
            }
